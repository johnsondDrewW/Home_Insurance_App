...
<body>
	...
	<% Policy policy = (Policy)session.getAttribute("policy");
		if(policy !=null){
			   %>
				<p align = "center"><%=request.getParameter("message") %></p>
			<table align = "center">
				<tr><td>Qoute ID</td><td><%= policy.getPolicy_ID()%></td></tr>
				<tr><td>Policy ID</td><td>PL<%= policy.getPolicy_ID()%>(0^0)</td></tr>
				<tr><td>Start Date</td><td><%=policy.getStart()%></td></tr>
				<tr><td>End Date</td>
				<%java.sql.Date d = new java.sql.Date(policy.getStart().getTime());
					d.setYear(d.getYear()+1);%>
				<td><%=d%></td></tr>
				<tr><td>Term length</td><td><%=policy.getTerm()%></td></tr>
				<tr><td>Status</td><td><%=policy.getStatus()%></td></tr>
				<%} %>
